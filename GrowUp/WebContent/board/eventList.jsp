			...
			<div class="row">
				...
				<c:if test="${sessionScope.grade == 'A' || sessionScope.grade == 'M'}">
					<div class="col-xs-4 text-right">
						<button type="button" class="btn btn-lg boardbtn" onclick="fn_eventWrite()">새 글 쓰기</button>
					</div>
				</c:if>
			</div>
		</div>
                ...
                <table class="table table-hover boardlist">
                    <tbody>
                        <c:forEach items="${dsEventList}" var="dsEventList">
                         <tr>
                         	<td>
                         		<div class="col-md-10"><a href ="${context}/work/board/eventView.do?evNo=${dsEventList.EV_NO}">${dsEventList.EV_TITLE}</a></div>
								<div class="col-md-2 text-right">${dsEventList.USER_NAME}</div>
                         	</td>
                         </tr>
                    	</c:forEach>
                    </tbody>
